...
class Simple_Settings {
	...
	public function select_callback( $args ) {
		...
		echo '<div class="simple-settings-select-' . ( ( $args['multiple'] ) ? 'multiple-' : '' ) . 'wrap">';
		echo '<select ' . esc_attr( $nonce ) . 'id="' . esc_attr( $this->func ) . '_settings[' . esc_attr( $id ) . ']" name="' . esc_attr( $name_attr ) . '" class="' . esc_attr( $class ) . ' ' . esc_attr( $select2 ) . '" data-dropdown-css-class="simple-settings-select2-dropdown" data-selection-css-class="simple-settings-select2-selection" data-placeholder="' . esc_attr( $placeholder ) . '"' . esc_attr( $width ) . ( ( $args['multiple'] ) ? ' multiple="true"' : '' ) . ' />';
	...
	public function enqueue_scripts( $hook ) {
		...
		$select2_cdn = 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/';
		...
		wp_enqueue_style( 'select2', $select2_cdn . 'css/select2.min.css', array(), '4.0.13' );
		wp_enqueue_script( 'select2', $select2_cdn . 'js/select2.full.min.js', array( 'jquery' ), '4.0.13', true );
